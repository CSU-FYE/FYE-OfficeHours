...
from simplify_workbook import OFFICE_ROOM, TUTORING_ROOM, sort_shifts
...
# read and dropped, and every row is written into OFFICE_ROOM instead.
...
def main():
    ...
    col = {n: column_of(shifts, n) for n in ("name", "day", "start", "end", "location")}

    # Tutoring is add_tutoring.py's to maintain, and the only thing that marks a row
    # as theirs is the room it names.
    def is_office_hours(row):
        return s(shifts.cell(row, col["location"]).value).lower() != TUTORING_ROOM.lower()
    ...
    if rooms and IGNORE_GRID_ROOMS:
        print(f"  note: ignored room text on the grid ({', '.join(rooms)}); these rows go "
              f"into {OFFICE_ROOM}")
    for name in dropped:
    ...
    keep = [[shifts.cell(row, c).value for c in range(1, width + 1)]
            ...
            if not (is_office_hours(row) and s(shifts.cell(row, col["name"]).value) in grid_names)]

    block = []
    for name, day, start, end in wanted:
        values = [None] * width
        for column, value in (("name", name), ("day", day), ("start", start),
                              ("end", end), ("location", OFFICE_ROOM)):
            values[col[column] - 1] = value
        block.append(values)

    shifts.delete_rows(2, shifts.max_row)
    for i, values in enumerate(keep + block):
        for c, value in enumerate(values, start=1):
            cell = shifts.cell(2 + i, c, value)
            if c in (col["start"], col["end"]):
                cell.alignment = Alignment(horizontal="left")
    # Back into the order the week runs, on real clock minutes rather than text — which
    # would file 9 AM after 10 AM — with the filter stretched over the result.
    sort_shifts(shifts)
